...
import static com.foodsquad.FoodSquad.config.utils.Constant.ORDER_STATUS_COMPLETED;
...
public class ClientOrderServiceImpl implements ClientOrderService {
    ...
    public ClientOrderDTO placeOrder(ClientOrderDTO clientOrderDTO) {
        ...
        if (!ObjectUtils.isEmpty(clientOrderDTO.getCustomer()) && !clientOrderDTO.getSource().equals(OrderSource.POS)) {
            ...
            orderStatus = orderStatusService.getByCode(ORDER_STATUS_PENDING);
        }
        ...
        if (OrderSource.POS.equals(clientOrderDTO.getSource())) {
            ...
            order.setStatus(orderStatusService.getByCode(ORDER_STATUS_COMPLETED));
    ...
    @Override
    public PaginatedResponseDTO<ClientOrderDTO> getOrdersBySessionId(UUID sessionId, Pageable pageable) {
        log.info("Fetching client orders for cashier session ID: {}", sessionId);

        Page<Order> ordersPage = orderRepository.findByCashierSessionIdOrderByCreatedAtDesc(sessionId, pageable);

        List<ClientOrderDTO> orderDTOs = ordersPage.getContent().stream()
                .map(clientOrderMapper::toDto)
                .toList();
